export const generateNodejsExample = (data: any): string => {
  ...
  if (data.pdfPassword && Object.keys(data.pdfPassword).length > 0) {
    ...
    const props: string[] = []
    if (p.userPassword) props.push(`    userPassword: '${p.userPassword}'`)
    if (p.ownerPassword) props.push(`    ownerPassword: '${p.ownerPassword}'`)
    if (p.restrictPrinting) props.push(`    restrictPrinting: true`)
    if (p.restrictCopying) props.push(`    restrictCopying: true`)
    if (p.restrictModifying) props.push(`    restrictModifying: true`)
    if (props.length > 0) passwordFormatted = props.join(',\n')
  }

  // Format watermark
  let watermarkFormatted = ''
  if (data.watermark && Object.keys(data.watermark).length > 0) {
    const w = data.watermark
    const props: string[] = []
    if (w.text) props.push(`    text: '${w.text}'`)
    if (w.fontFamily) props.push(`    fontFamily: '${w.fontFamily}'`)
    if (w.fontWeight != null) props.push(`    fontWeight: ${w.fontWeight}`)
    if (w.fontItalic != null) props.push(`    fontItalic: ${w.fontItalic}`)
    if (w.fontSize != null) props.push(`    fontSize: ${w.fontSize}`)
    if (w.color) props.push(`    color: '${w.color}'`)
    if (w.opacity != null) props.push(`    opacity: ${w.opacity}`)
    if (w.rotation != null) props.push(`    rotation: ${w.rotation}`)
    if (w.positionX) props.push(`    positionX: '${w.positionX}'`)
    if (w.positionY) props.push(`    positionY: '${w.positionY}'`)
    if (w.pages && w.pages.length > 0) props.push(`    pages: [${w.pages.join(', ')}]`)
    if (props.length > 0) watermarkFormatted = props.join(',\n')
  }
