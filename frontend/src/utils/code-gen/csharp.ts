export const generateCsharpExample = (data: any, indent = 4): string => {
  ...
  if (data.pdfPassword && Object.keys(data.pdfPassword).length > 0) {
    const p = data.pdfPassword
    csharpStr += padInner + 'PdfPassword = new()\n' + padInner + '{\n'
    if (p.userPassword) csharpStr += padInner2 + `UserPassword = "${p.userPassword}",\n`
    if (p.ownerPassword) csharpStr += padInner2 + `OwnerPassword = "${p.ownerPassword}",\n`
    if (p.restrictPrinting) csharpStr += padInner2 + `RestrictPrinting = true,\n`
    if (p.restrictCopying) csharpStr += padInner2 + `RestrictCopying = true,\n`
    if (p.restrictModifying) csharpStr += padInner2 + `RestrictModifying = true,\n`
    if (csharpStr.endsWith(',\n')) csharpStr = csharpStr.slice(0, -2) + '\n'
    csharpStr += padInner + '},\n'
  }

  if (data.watermark && Object.keys(data.watermark).length > 0) {
    const w = data.watermark
    csharpStr += padInner + 'Watermark = new()\n' + padInner + '{\n'
    if (w.text) csharpStr += padInner2 + `Text = "${w.text}",\n`
    if (w.fontFamily) csharpStr += padInner2 + `FontFamily = "${w.fontFamily}",\n`
    if (w.fontWeight != null) csharpStr += padInner2 + `FontWeight = ${w.fontWeight},\n`
    if (w.fontItalic != null) csharpStr += padInner2 + `FontItalic = ${w.fontItalic ? 'true' : 'false'},\n`
    if (w.fontSize != null) csharpStr += padInner2 + `FontSize = ${w.fontSize},\n`
    if (w.color) csharpStr += padInner2 + `Color = "${w.color}",\n`
    if (w.opacity != null) csharpStr += padInner2 + `Opacity = ${w.opacity},\n`
    if (w.rotation != null) csharpStr += padInner2 + `Rotation = ${w.rotation},\n`
    if (w.positionX) csharpStr += padInner2 + `PositionX = "${w.positionX}",\n`
    if (w.positionY) csharpStr += padInner2 + `PositionY = "${w.positionY}",\n`
    if (w.pages && w.pages.length > 0) csharpStr += padInner2 + `Pages = new() { ${w.pages.join(', ')} },\n`
    if (csharpStr.endsWith(',\n')) csharpStr = csharpStr.slice(0, -2) + '\n'
    csharpStr += padInner + '},\n'
  }
